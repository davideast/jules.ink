...
function Logo({ onClick }: { onClick?: () => void }) {
  const content = (
    <>
      ...
      <span className="text-white/60 text-[13px] font-mono tracking-[-0.02em]">
        jules.ink
      </span>
    </>
  );
  ...
  if (onClick) {
    return (
      <button className="flex items-center gap-3 hover:opacity-80 transition-opacity" onClick={onClick}>
        {content}
      </button>
    );
  }
...
export function TopBar({
  ...
  return (
    <header className="h-[52px] bg-sidebar-bg border-b border-[#2a2a35] flex items-center justify-between px-4 shrink-0 z-30">
      {/* Left: logo + session context */}
      <div className="flex items-center gap-3 min-w-0">
        <Logo onClick={onSessionClose} />
        {sessionId && sessionTitle ? (
          <>
            <div className="h-4 w-px bg-[#2a2a32] shrink-0" />
            <span className="text-soft-white text-[13px] font-medium truncate">
              {sessionTitle}
            </span>
            {sessionRepo ? (
              <>
                <span className="text-[#72728a] text-[13px] shrink-0">&middot;</span>
                <span className="font-mono text-[#72728a] text-[12px] truncate">
                  {sessionRepo}
                </span>
              </>
            ) : null}
        ...
        {sessionId ? (
          <div className="flex items-center gap-4">
            ...
            {sessionState === 'streaming' ? (
              ...
              <div className="flex items-center gap-2">
                <span className="material-symbols-outlined text-[14px] text-[#72728a]" style={FILLED}>
                  play_arrow
                </span>
                <span className="text-[#72728a] text-xs font-medium">
                  Ready
