...
export const LabelPreview = memo(function LabelPreview({
  ...
  const bodyFontSize = getBodyFontSize(summary);
  const visibleFiles = files.slice(0, 5);
  const hiddenCount = files.length - 5;

  return (
    <div className="w-full h-full flex flex-col text-black select-none">
      {/* Header: repo + sessionId */}
      <div className="flex justify-between items-baseline px-[18px] pt-[14px] gap-2">
        <span className="font-mono text-[9px] text-black/60 truncate">
          {repo}
        </span>
        <span className="font-mono text-[9px] text-black/60 shrink-0">
          {sessionId}
        </span>
      </div>
      ...
      {visibleFiles.length > 0 ? (
        <div className="px-[18px] pb-1 flex flex-col gap-[4px]">
          {visibleFiles.map((f) => (
            <div
              key={f.path}
              className="flex justify-between items-baseline font-mono text-[10px]"
            >
              <span className="truncate mr-2 text-black">
                {truncateMiddle(f.path, 28)}
              </span>
          ...
          {hiddenCount > 0 ? (
            <div className="text-center text-[9px] text-black/40 italic font-display">
